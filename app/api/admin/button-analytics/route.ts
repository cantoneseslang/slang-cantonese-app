...
export async function GET() {
  ...
  if (evErr) {
    ...
    if (evErr.code === 'PGRST116' || evErr.message?.includes('relation') || evErr.message?.includes('does not exist')) {
      return NextResponse.json({ success: true, total_buttons: 0, users: [] })
    }
