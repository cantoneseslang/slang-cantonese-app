    ...
    if (existingSubscriptionId) {
      try {
        ...
        if (existingSubscription.status === 'active' || existingSubscription.status === 'trialing') {
          ...
          // ⚠️ 重要: プランが異なる場合は、決済を確実にするためCheckout Sessionを作成
          // subscriptions.update()は決済失敗の可能性があるため、Checkout Sessionを使用して決済を確実にする
          console.log('🔄 プラン変更が検出されました。決済を確実にするためCheckout Sessionを作成します:', {
            subscriptionId: existingSubscriptionId,
            currentPriceId,
            newPriceId,
            plan
          ...
          // 既存サブスクリプションは、Checkout Session完了後にWebhookで更新される
        }
      } catch (err: any) {
            ...
            try {
              ...
              if (foundSubscription.status === 'active' || foundSubscription.status === 'trialing') {
                ...
                if (pricingError || !pricingConfig) {
                  ...
                  // ⚠️ 重要: プランが異なる場合は、決済を確実にするためCheckout Sessionを作成
                  // subscriptions.update()は決済失敗の可能性があるため、Checkout Sessionを使用して決済を確実にする
                  console.log('🔄 プラン変更が検出されました（メールアドレスから発見）。決済を確実にするためCheckout Sessionを作成します:', {
                    subscriptionId: existingSubscriptionId,
                    currentPriceId,
                    newPriceId,
                    plan
