...
export default function Home() {
  ...
  const runOcr = async (file: File, onProgress?: (p: number) => void): Promise<string> => {
    ...
    try {
      ...
      // より積極的にスペースを削除
      text = text
        // すべての中国語・日本語文字間のスペースを削除（句読点含む）
        .replace(/([\u4e00-\u9fff\u3400-\u4dbf\u3040-\u309f\u30a0-\u30ff，。、．])\s+([\u4e00-\u9fff\u3400-\u4dbf\u3040-\u309f\u30a0-\u30ff，。、．])/g, '$1$2')
        // 連続するスペースを1つに（英文などが含まれる場合のため）
        .replace(/\s{2,}/g, ' ')
        // 句読点前後のスペースを削除
        .replace(/\s+([，。、．])/g, '$1')
        .replace(/([，。、．])\s+/g, '$1')
        // タイトル部分（「光輝歲月 黃家駒」）のスペースは保持するため、特定パターンは除外
        // ただし、OCR結果では不規則なので、すべての文字間スペースを削除
        .trim();
      
      // 再度、残っている中国語・日本語文字間のスペースを削除（念のため）
      text = text.replace(/([\u4e00-\u9fff\u3400-\u4dbf\u3040-\u309f\u30a0-\u30ff])\s+([\u4e00-\u9fff\u3400-\u4dbf\u3040-\u309f\u30a0-\u30ff])/g, '$1$2');
